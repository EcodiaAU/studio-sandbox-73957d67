...
import { ContactFormWire } from './ContactFormWire';

export interface ContactFormProps {
  heading: string;
  intro?: string;
  /** Small kicker line above the heading, e.g. "Contact". */
  eyebrow?: string;
  /** Button label. */
  submitLabel?: string;
  /** Placeholders for the three fields. */
...
export function ContactFormSection(props: ContactFormProps): ReactElement {
  const { heading, intro } = props;
  const eyebrow = props.eyebrow || 'Contact';
  const t = useTheme(props.theme);
  return (
    <section data-eos-id="app/_generated/ContactFormSection.tsx#0" data-eos-v="2" className="px-6 py-20 sm:py-28" style={{ backgroundColor: t.surface, color: t.ink }}>
      <div data-eos-id="app/_generated/ContactFormSection.tsx#1" style={container(t)}>
        <div data-eos-id="app/_generated/ContactFormSection.tsx#2" className="mx-auto" style={{ maxWidth: '38rem' }}>
          <div data-eos-id="app/_generated/ContactFormSection.tsx#3" className="text-center">
            {eyebrow ? (
              <p data-eos-id="app/_generated/ContactFormSection.tsx#4" className="text-xs font-semibold uppercase" style={{ color: t.primary, letterSpacing: '0.18em' }}>{eyebrow}</p>
            ) : null}
            <h2 data-eos-id="app/_generated/ContactFormSection.tsx#5" className="mt-3 text-3xl font-semibold tracking-tight sm:text-4xl">{heading}</h2>
            {intro ? (
              <p data-eos-id="app/_generated/ContactFormSection.tsx#6" className="mt-4 text-lg leading-8" style={{ color: t.muted }}>{intro}</p>
            ) : null}
          </div>
          <ContactFormWire data-eos-id="app/_generated/ContactFormSection.tsx#7"
            submitLabel={props.submitLabel || 'Send message'}
            namePlaceholder={props.namePlaceholder || 'Your name'}
            emailPlaceholder={props.emailPlaceholder || 'you@example.com'}
            messagePlaceholder={props.messagePlaceholder || 'How can we help?'}
